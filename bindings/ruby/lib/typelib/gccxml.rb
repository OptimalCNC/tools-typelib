...
module Typelib
    ...
    class GCCXMLInfo
        ...
        STORED_TAGS = %w{Namespace Typedef Enumeration Struct Class ArrayType FundamentalType PointerType CvQualifiedType}.freeze
        ...
        def initialize(required_files)
            ...
            @id_to_node = {}
            @name_to_nodes = Hash.new { |h, k| h[k] = [] }
            @virtual_members = Set.new

            @bases = Hash.new { |h, k| h[k] = [] }
            @enum_values = Hash.new { |h, k| h[k] = [] }

            @types_per_file = Hash.new { |h, k| h[k] = [] }
            @typedefs_per_file = Hash.new { |h, k| h[k] = [] }
        end

        def file(attributes)
            file_name = attributes["name"]
            required_files[file_name] = attributes["id"] if required_files.key?(file_name)
        end
        ...
        def tag_start(name, attributes)
            ...
            elsif name == "Field" || name == "FundamentalType" # rubocop:disable Style/MultipleComparison
                id_to_node[attributes["id"]] = Node.new(name, attributes)
            elsif name == "Base"
            ...
            return unless STORED_TAGS.include?(name)
        ...
        def parse(xml)
            ...
            if root_tag !~ /<GCC_XML/
                raise "the provided XML input does not look like a GCCXML output (expected a root GCC_XML tag but got #{root_tag.chomp})"
            end
    ...
    class GCCXMLLoader
        ...
        def self.tokenized_cxx_to_typelib(tokens, &filter)
            ...
            result = result.join("")
            result = "/#{result}" if result[0, 1] != "/" && result !~ /^[-+\d]/
            result
        end
        ...
        def self.split_last_namespace(name)
            ...
            [typename.reverse, basename.reverse]
        end

        NAMESPACE_NODE_TYPES = %w{Namespace Struct Class}.freeze
        ...
        def resolve_namespace_of(name)
            ...
            while true
                ...
                context = if !context
                              ...
                              candidates.find { |node| node["context"].to_s == context }
                          end
                if !context
                    break
        ...
        def resolve_qualified_type(xmlnode)
            spec = []
            spec << "const" if xmlnode["const"] == "1"
            spec << "volatile" if xmlnode["volatile"] == "1"
        ...
        def source_file_content(file)
            if source_file_contents.key?(file)
                source_file_contents[file]
            else
                source_file_contents[file] =
                    (File.readlines(file, encoding: "utf-8") if File.file?(file))
            end
        end
        ...
        def resolve_type_definition(xmlnode)
            ...
            if kind == "Struct" || kind == "Class" # rubocop:disable Style/MultipleComparison
                ...
                if type_name == "/std/string"
                    ...
                    unless registry.include?(contained_type)
                        contained_node = find_node_by_name(contained_type)
                        unless contained_node
                            contained_node = find_node_by_name(contained_type)
                    ...
                    type.metadata.set("cxxname", cxxname)
                    set_source_file(type, xmlnode)
                    registry.alias(name, type.name) if name != type.name
                    ...
                    base_classes = info.bases[xmlnode["id"]].map do |child_node|
                        ...
                        unless base_type_name
                            unless name.start_with?("/std/allocator")
                                msg = "ignoring #{name}, it has ignored base classes"
                            end
                    ...
                    if xmlnode["incomplete"] == "1"
                        unless name.start_with?("/std/allocator")
                            msg = "ignoring incomplete type #{name}"
                        end
                        return ignore(xmlnode, msg)
                    end
                    ...
                    fields = member_ids.map do |member_id|
                        if field_node = info.id_to_node[member_id]
                            field_node if field_node.name == "Field"
                        end
                    end.compact

                    if fields.empty? && base_classes.all? { |type, _| type.empty? }
                        unless name =~ %r[^/[^/]+/new_allocator]
                            msg = "ignoring the empty struct/class #{name}"
                        end
                    ...
                    type.metadata.set("cxxname", cxxname)
                    registry.alias(name, normalized_name) if name != normalized_name
                    set_source_file(type, xmlnode)
                ...
                unless (pointed_to_type = resolve_type_id(xmlnode["type"]))
                ...
                registry.alias(name, pointed_to_name) if name != pointed_to_name
                if normalized_name != pointed_to_name
                    registry.alias(normalized_name, pointed_to_name)
                    unless Typelib.basename(normalized_name).start_with?("__")
        ...
        def load(required_files, xml)
            ...
            registry.each do |t|
                unless base_registry.include?(t.name)
            ...
            registry.each(with_aliases: true) do |name, t|
                ...
                filtered_registry.alias(name, t.name) if permanent_alias?(name)
            end
            filtered_registry
        end
        ...
        def extract_doc_metadata(metadata, doc)
            meta = doc.split("\n").grep(/^\s*@meta/)
            meta.each do |line|
                metadata.add($1, $2.strip) if line =~ /^\s*@meta (\w+)\s+(.*)$/
            end
        end
